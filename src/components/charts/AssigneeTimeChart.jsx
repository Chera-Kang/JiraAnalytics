  ...
  if (active && payload && payload.length) {
    return (
      <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '12px', boxShadow: 'var(--shadow-md)' }}>
        ...
        <span style={{ color: 'var(--text-secondary)' }}>평균 처리 일수: </span>
        <strong style={{ color: 'var(--text-primary)' }}>{payload[0].value}일</strong>
      </div>
    );
  }
  return null;
...
/**
 * [담당자별 평균 이슈 처리 일수 수평 바 차트]
 * - 카드 높이는 390px로 고정하고, 막대 두께만 인원수에 따라 가변적으로 조절
 */
const AssigneeTimeChart = ({ data }) => {
